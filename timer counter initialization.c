...
void timer_counter_C0_C1_D0_init(uint16_t topcount) {
    ...
    // ...so, let's do it...
    // Set the data direction for the PORT C PWM pins.
    ...
    TCC0_CTRLD = TC_TC0_EVSEL_OFF_gc | TC_TC0_EVSEL_OFF_gc;
    TCC1_CTRLD = TC_TC1_EVSEL_OFF_gc | TC_TC1_EVACT_OFF_gc;
...
void timer_counter_F0_init(uint16_t topcount) {
	//
	// This routine is the production version of the light initialization. It uses pins 0-2 on port F.
	PORTF_DIR=0b00000111;
	//
	// Set the clock prescaler to 64. From table 14-3:
	TCF0_CTRLA = TC_TC0_CLKSEL_DIV64_gc;
	//
	// Enable the compare enable functions: See 14.12.2
	// Set WGM=0b011, which is single slope PWM.  See 14.12.2
	TCF0_CTRLB = TC0_CCAEN_bm | TC0_CCBEN_bm  | TC0_CCCEN_bm | TC0_CCDEN_bm | TC_TC0_WGMODE_SINGLESLOPE_gc;
	//
	// Ensure event actions are turned OFF
	TCF0_CTRLD = TC_TC0_EVSEL_OFF_gc | TC_TC0_EVACT_OFF_gc;
	//
	// Nothing to set in TCCx control registers C, and E.
	//
	// Ensure timer/counters are enabled for port C (See 8.7.3)
	...
	for (char i=0;i<3;i++)
	{
		...
		TCF0_CCB = light_lowest_setting;
		TCF0_CCC = light_lowest_setting;
		_delay_ms(1000);
		TCF0_CCA = light_middle_setting;
		TCF0_CCB = light_middle_setting;
		TCF0_CCC = light_middle_setting;
		_delay_ms(1000);
		TCF0_CCA = light_highest_setting;
		TCF0_CCB = light_highest_setting;
